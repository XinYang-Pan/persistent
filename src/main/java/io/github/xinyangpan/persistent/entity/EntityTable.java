package io.github.xinyangpan.persistent.entity;
...
public abstract class EntityTable<T> {
	...
	private void validateAfterInit() {
		//
		Preconditions.checkNotNull(this.getParameterizedClass());
		Preconditions.checkNotNull(this.getIdCol());
		Assert.hasText(this.getTableName(), "Table name must have text.");
		Assert.notEmpty(this.getNoneIdCols(), "NoneIdCols can not be empty.");
	}
